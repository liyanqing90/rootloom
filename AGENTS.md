...
- `plugins/rootloom/skills/setup-rootloom/scripts/setup_rootloom.py` owns Codex-home writes and capability-to-artifact mapping; it must remain plan-first, dependency-closing, lock-serialized, fully compensating, mode-preserving, conflict-refusing, atomic, backed up, and hash-aware.
...
- `plugins/rootloom/skills/high-assurance-coding-change/scripts/run_pipeline.py` owns the strict local stage graph; changes to its Git-state, provenance references, path, structured-output, ignored-path metadata, or repair-cycle gates require regression coverage in its bundled test module.
