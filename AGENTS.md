...
- `plugins/rootloom/lib/rootloom_paths.py` owns separate secret-material privacy and security-domain risk classification, and `plugins/rootloom/lib/rootloom_memory.py` owns strict Engineering Memory reads, schema, identity, relevance, and lifecycle semantics. Capture must use only the material classifier for redaction; Analyzer uses the domain classifier for risk. Consumers must not grow divergent copies. See `docs/decisions/2026-07-15-sensitive-material-and-capture-bounds.md`.
