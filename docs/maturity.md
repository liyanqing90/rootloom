...
`HUMAN_REVIEW_REQUIRED` now has a v4 accept/reject command bound to an exact Result Envelope v1, final validated worktree/index/Git-control commitment, exact-missing protected targets and parent boundaries, canonical mode-0700 run identity, complete metadata-only floor, bounded private Artifact hashes, and canonical reviewer/local identity and UTC time. The Envelope proves a nonempty zero-repair deletion-only change set, allowed paths, unchanged index, matching metadata records, bound Delta/metadata Artifacts, and Runner/run identity; older incomplete Results fail closed. Terminal and Summary share a 1 MiB producer/consumer limit and stay pinned through final exact-byte-count/SHA-256 validation. The verifier has four claims: `INVALID`/9 for malformed or unsafe evidence, `STALE`/12 only after completed recapture proves current-state difference, and `UNVERIFIED`/13 when Git, permission/I/O, topology, deadline, or independent local resource ceilings prevent a conclusion, while `VALID` remains 0. Persisted production budgets cannot raise consumer limits. Git recapture disables optional locks, fsmonitor, and untracked-cache writes, with filesystem-state regressions covering the repository, index/control files, lock paths, worktree, and Run Directory. Typed errors/contracts/Git behavior live under neutral Runner modules and Human Review receives remaining primitives explicitly rather than importing `run_pipeline.py`. The hardened cooperative lock and repeated checks still cannot stop arbitrary same-UID writers or replacement of the locked pathname with a new inode. Human Review v4 therefore remains attributable final review for trusted personal or small-team environments, not hostile local approval or cryptographically signed/WORM organizational evidence; stronger governance belongs to immutable snapshots, independent workers or UIDs, external signers, remote immutable Artifact stores, and WORM audit systems.
...
The strict high-assurance runner supports Linux, macOS, and WSL and rejects native Windows. The v1.2.18 CI matrix runs full Linux checks on Python 3.11–3.14, macOS Strict Runner tests, portable setup/guidance/hook and hardened-cooperative-lock tests on macOS and Windows, and the pinned Codex CLI compatibility smoke; exact POSIX permission-mode assertions remain Linux/macOS-only because Windows exposes ACL/read-only semantics rather than `fchmod`. Release run `29237033241` passed all eight jobs against exact tagged commit `16b9966a3512d46014da32196f331a73246ea4e3`. Native custom-role/model routing is still not attested on the currently verified spawn surface, so the sequential runner is the supported route when model assignment must be evidenced.
