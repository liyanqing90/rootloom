...
`HUMAN_REVIEW_REQUIRED` now has a v4 accept/reject command bound to a bounded canonical worktree/index/Git-control commitment, exact-missing protected targets and parent boundaries, canonical run-directory identity, the complete final metadata-only floor, bounded private Artifact hashes, and reviewer/local identity. Result is read through stable no-follow descriptors before the append, after it, and after the repeated commitment; a copied run, Result mutation, repository drift, or protected-path declassification refuses or compensates the decision. Per-file/aggregate Artifact bytes, count, binding time, and Result size are bounded. The decision record is append-only within the private run directory and refuses duplicate terminal decisions. The cooperative lock and repeated checks cannot stop arbitrary external writers, and the record is not cryptographically signed organizational identity or storage-level immutability; production governance may still require an immutable snapshot, external signer, or WORM audit store.
...
The strict high-assurance runner supports Linux, macOS, and WSL and rejects native Windows. The v1.2.14 CI matrix runs full Linux checks on Python 3.11–3.14, macOS Strict Runner tests, portable setup/guidance/hook and shared-lock tests on macOS and Windows, and the pinned Codex CLI compatibility smoke; exact POSIX permission-mode assertions remain Linux/macOS-only because Windows exposes ACL/read-only semantics rather than `fchmod`. Release run `29220934659` passed all eight jobs against exact tagged commit `7cc3bb9f5de69b13c77f1c06e499a8328e095204`. Native custom-role/model routing is still not attested on the currently verified spawn surface, so the sequential runner is the supported route when model assignment must be evidenced.
