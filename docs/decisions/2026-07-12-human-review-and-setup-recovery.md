...
- Human Review v4 uses versioned binding and decision records over the canonical final Result core and canonical run-directory identity; the complete final validated metadata-only floor; an exact canonical commitment to visible content fingerprints, metadata-only fingerprints, status/path sets, Git index, and Git control state; each authorized deletion's exact-missing state and lexical parent boundaries; reviewed private Artifact hashes with count, exact-name-set, per-file, aggregate-byte, and time budgets; asserted reviewer plus local account/uid; and decision time. Initial Binding reuses the final validated floor before content reads and refuses any commitment mismatch instead of adopting a later state. Accept/reject reads Result through a bounded directory-relative no-follow descriptor, rejects v2/v3 or a malformed v4 policy/floor before repository capture, and computes the pre-write commitment under the repository lock. Each Result and reviewed Artifact read verifies before and after that its canonical name still identifies the regular single-link descriptor whose stable bytes were consumed; Artifact enumeration must also return the same exact names before and after hashing. Terminal and Summary must both be regular single-link files opened under a pinned Run Directory; their pinned descriptors own append, fsync, final validation, and compensation. The final gate recomputes exact byte counts and SHA-256 through those descriptors and requires canonical name, inode, one link, mode, size, and original payload digest to match. Any earlier failure truncates both pinned original inodes to an empty retryable state. Existing v2/v3 results and older non-empty Terminal-without-Summary states fail closed rather than being silently upgraded, inferred, or rewritten.
- A read-only Decision Pair verifier reports `VALID`/0, `INVALID`/9, `STALE`/12, or `UNVERIFIED`/13. Invalid is reserved for malformed, conflicting, noncanonical, or unsafe persisted evidence; stale requires a completed recapture that proves current repository, Artifact, or protected-deletion difference; unverified means Git, permission/I/O, topology, deadline, or independent local resource ceilings prevented a conclusion. Internal Pipeline exit codes are not part of the public protocol.
- Human Review requires exact Result Envelope v1 before Binding or Pair evaluation. It proves one nonempty canonical zero-repair deletion-only change set, unchanged Git index, complete allowed paths, exact metadata records, bound Delta/metadata Artifacts, and Runner/run identity. Binding schema additionally constrains counts against persisted state budgets and requires a mode-0700 Run Directory. Older incomplete Results fail closed and must be regenerated.
- Persisted policy records producer limits but never authorizes verifier work. Independent verifier ceilings govern ignored/state paths, state bytes, per/total Artifact bytes, and time; recorded values above a local ceiling yield `UNVERIFIED` before recapture. Verifier Git subprocesses run with optional locks disabled and command-scoped fsmonitor/untracked-cache disabling.
- Decision Pair producer and consumer share one 1 MiB per-file budget. Reviewer/local-account identities share one canonical control-free 4096 UTF-8-byte schema, local UID is null or nonnegative, and decision time is canonical UTC. Human Review composition depends on neutral Runner errors/contracts/Git policy plus an explicit runtime interface and does not import `run_pipeline.py`.
- Setup, project Guidance, and Strict Runner repository locks use the shared hardened cooperative `plugins/rootloom/lib/rootloom_lock.py` boundary. POSIX opens the target relative to a stable no-follow parent descriptor; Windows uses reparse-point-aware parent and target handles. Both require a direct regular single-link target with stable identity, acquire non-blockingly, and only then truncate/write owner data. This inode protocol is not an irreplaceable lease: a same-UID process can rename/unlink the locked inode and lock a replacement created at the same pathname.
...
- Treat inode, size, and metadata as proof of the final Decision Pair payload — rejected because an external descriptor can overwrite equal-length bytes without changing the inode or size.
- Continue adding local TOCTOU checks against arbitrary same-UID processes — rejected because no last local check can prevent a hostile writer from acting afterward; stronger guarantees require a separate immutable or signed boundary.
- Convert every Binding recomputation error to `STALE` — rejected because malformed persisted evidence must be distinguished from a previously valid commitment whose external state changed.
- Convert every observation failure to `INVALID` — rejected because a transient environment or I/O failure does not prove persisted evidence invalid. `UNVERIFIED` preserves the absence of a conclusion.
- Let recorded production budgets control verifier work — rejected because untrusted evidence cannot select consumer resource cost; operator-owned local ceilings are a separate trust boundary.
- Infer a complete Runner Result from a minimal v4 object — rejected because missing workflow provenance and deletion-only invariants cannot be reconstructed after production.
- Let Producer output exceed the verifier's read budget — rejected because a successful local state transition must be consumable by the same version; payload limits belong before file creation.
- Move general repository capture and every low-level descriptor primitive during the Human Review split — rejected because that broad rewrite would duplicate or destabilize already-tested Runner security boundaries without changing the classification decision.
...
- Result Envelope v1 is intentionally required even with Binding v4. Pre-Envelope Results remain historical evidence for their producing version but are invalid to the new verifier and must not be rewritten.
- The hardened cooperative repository lock serializes cooperative Rootloom writers only. A same-UID process can replace the locked pathname with a new inode; post-write checks narrow but cannot eliminate arbitrary external-process TOCTOU.
- Human Review v4 is frozen as attributable final review for trusted personal or small-team environments. Hostile approval or stronger organizational assurance requires read-only snapshots, independent workers or UIDs, external signatures, a remote immutable Artifact store, or WORM audit storage.
...
- Runner tests cover ignored deletion-target recreation refusal, canonical state commitments, final-checkpoint equality and full metadata-floor reuse after ignore declassification, repository-lock contention, copied-run refusal, post-write repository and Result drift compensation, terminal hardlink/path-replacement victim preservation, equal-length same-inode Terminal/Summary mutation with pinned compensation and retry, Decision Pair VALID/INVALID/STALE/UNVERIFIED classification, complete Result Envelope and canonical Decision fields, consumer ceilings, read-only Git environment plus whole-filesystem snapshots, no-follow source-bounded Result/Artifact reads, Artifact per-file/aggregate/deadline limits, full-capacity binding, durable acceptance, launcher identity drift, selector-initialization cleanup, and duplicate terminal refusal.
...
## 2026-07-13 v4 content-integrity and scope-freeze amendment

The prior final output check still trusted inode, size, mode, and metadata, so an already-open external descriptor could replace Terminal or Summary bytes in place with equal-length content. The final commit gate now reads the exact expected byte count and SHA-256 through each pinned descriptor and compensates both files on any mismatch. A separate read-only verifier checks an already-persisted pair against Result, repository, protected-deletion, and Run Directory state, distinguishing invalid evidence from structurally valid but stale evidence. These are compatible v4 corrections because no persisted field or interpretation changes. With exact local payload hashing complete, v4 stops expanding toward hostile same-UID approval; stronger guarantees move to external immutable execution, identity, Artifact, and audit systems.

## 2026-07-13 v4 schema-first and module-ownership amendment

The first verifier delegated several persisted Binding shapes to current-state recomputation and returned the caught internal `PipelineError.exit_code`. Consequently `INVALID` could exit 8, and malformed evidence could be mislabeled `STALE`. The verifier now completes exact v4 schema and internal-commitment validation before recapture, collapses every invalid path to public exit 9, and reserves exit 12 for current-state drift after that gate. Producer and consumer use the same Pair budget before creation/after opening, identities are bounded, and failure reasons move to bounded stderr while stdout stays one status word. Schema, binding orchestration, pair I/O composition, decision production, and verification classification have separate module owners; the persisted v4 field set and meanings do not change. Noncanonical or manually corrupted v4 evidence fails closed rather than being migrated or repaired.

## 2026-07-13 Result Envelope and typed-verification amendment

The schema-first verifier still converted every current-state recomputation failure to stale, trusted recorded limits as consumer policy, and accepted an incomplete Result outer object. Human Review now requires Result Envelope v1 while retaining Binding and Decision v4. Typed errors separate invalid evidence, completed drift, and inability to verify; `UNVERIFIED`/13 is additive to the public status protocol. Independent local ceilings cap recapture before work, and verifier Git commands disable optional writes without changing repository config. Shared errors/contracts/Git behavior move to neutral Runner modules, while Human Review receives remaining primitives explicitly and no longer imports `run_pipeline.py`. This is a deliberate fail-closed Result compatibility break: older incomplete Results are not migrated because their missing provenance cannot be safely inferred.
