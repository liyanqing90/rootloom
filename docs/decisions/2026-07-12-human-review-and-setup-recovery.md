...
- Human Review v4 uses versioned binding and decision records over the canonical final Result core and canonical run-directory identity; the complete final validated metadata-only floor; an exact canonical commitment to visible content fingerprints, metadata-only fingerprints, status/path sets, Git index, and Git control state; each authorized deletion's exact-missing state and lexical parent boundaries; reviewed private Artifact hashes with count, per-file, aggregate-byte, and time budgets; asserted reviewer plus local account/uid; and decision time. Initial Binding reuses the final validated floor before content reads and refuses any commitment mismatch instead of adopting a later state. Accept/reject reads Result through a bounded directory-relative no-follow descriptor, rejects v2/v3 or a malformed v4 policy/floor before repository capture, and computes the pre-write commitment under the repository lock. The terminal decision must be a regular single-link file whose directory entry and descriptor identity match; one pinned descriptor owns append, fsync, post-write identity validation, and compensation. Result is then safely re-read, the complete commitment is recomputed, and Result is read again; any Result, repository, or terminal identity drift truncates only the pinned original inode. Existing v2/v3 results fail closed rather than being silently upgraded from evidence they never captured.
