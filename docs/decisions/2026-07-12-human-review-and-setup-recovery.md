...
- A read-only Decision Pair verifier reports `VALID`/0, `INVALID`/9, or `STALE`/12. It checks the Result binding, one canonical Terminal record, canonical Summary agreement and Terminal hash, regular single-link files, current repository/protected-deletion commitment, and Run Directory identity without acquiring or modifying the repository lock.
- Human Review v4 schema validation precedes every repository or reviewed-Artifact recapture. It owns the exact Binding fields, repository/run identities, Git and Result hashes, canonical policy/path sets, Repository State Commitment hashes, bounded Artifact Map, protected-deletion parent commitments, and Binding/Result agreement. Structural, safety, budget, or internal-consistency failures are `INVALID`/9; only a fully valid pair whose current state differs is `STALE`/12. Internal Pipeline exit codes are not part of the public verifier protocol.
- Decision Pair producer and consumer share one 1 MiB per-file budget, and reviewer/local-account identities share a 4096 UTF-8-byte limit. Both canonical payloads are preflighted before either file is created. Human Review composition is divided into schema, binding, pinned pair I/O, decision transaction, and verification-classification modules; the Runner retains general repository capture and low-level descriptor primitives.
...
- Runner tests cover ignored deletion-target recreation refusal, canonical state commitments, final-checkpoint equality and full metadata-floor reuse after ignore declassification, repository-lock contention, copied-run refusal, post-write repository and Result drift compensation, terminal hardlink/path-replacement victim preservation, equal-length same-inode Terminal/Summary mutation with pinned compensation and retry, read-only Decision Pair VALID/INVALID/STALE classification, no-follow source-bounded Result/Artifact reads, Artifact per-file/aggregate/deadline limits, full-capacity binding, durable acceptance, launcher identity drift, selector-initialization cleanup, and duplicate terminal refusal.
