...
- a read-only Decision Pair verifier that validates the complete v4 persisted schema before recapture, reports malformed/unsafe/internal-conflict evidence as `INVALID`/9, and reserves `STALE`/12 for a structurally valid pair whose current repository, Artifact, or protected-deletion state differs; stdout is one status word and bounded stderr carries the reason;
...
Human Review composition is split under `scripts/human_review/`: `schema.py` owns persisted validation, `binding.py` owns current-state recomputation, `pinned_io.py` owns pair reads through stable private descriptors, `decision.py` owns identity/payload budgets and the compensating producer transaction, and `verify.py` owns stale classification. The top-level `review_decision.py` contains only CLI parsing, stable status mapping, bounded diagnostics, and compatibility exports. General repository capture and low-level descriptor primitives remain Runner-owned so the extraction does not duplicate security I/O.
