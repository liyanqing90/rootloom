...
- a Human Review v3 binding/decision protocol over the canonical final Result core, bounded worktree/index/Git-control commitments, exact-missing protected targets plus parent boundaries, private Artifact hashes captured through stable directory-relative no-follow descriptors, reviewer/local identity, repository-lock serialization, no-follow terminal records, and a compensating post-write drift check;
