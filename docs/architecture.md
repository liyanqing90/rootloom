...
- deterministic verification commands without a shell, stable command IDs, successful command-record coverage for every diagnosis verification item, at least one mapped user-supplied command beyond formatting-only `verify-0`, and fingerprint stability for detected verification entrypoints such as explicit repository-relative path tokens, `make` files, JavaScript package manifests, and pytest configuration files;
