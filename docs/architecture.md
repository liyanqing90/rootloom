...
- selector-based merged-output streaming into a bounded tail, with structured observed/retained byte counts, truncation, output-limit, drain-cutoff, and detached-descendant fields;
- immediate original-group cleanup and a one-second local drain when the direct parent exits with stdout still open, without consuming the remaining stage timeout;
- streamed complete-or-fail Git Delta artifacts with a 32 MiB aggregate default per capture and an additional 8 MiB ordinary-untracked patch default; Runner-owned capture shares the stage deadline/drain, disables external diff and textconv drivers, compensates partial writes, and loads only fixed prompt excerpts;
- structured per-model-stage command sidecars, a 32 MiB retained-output verification-batch budget, append-only NDJSON governed by a 64 MiB actual serialized-byte default with pre-materialization escape counting and pre-command minimum-record checks, a 64-command ceiling, and a 120,000-character compact verification prompt ceiling;
