...
When strict Tier 1/2 evidence is explicitly requested, `begin_review.py` creates an exclusive outside-repository intake directory with `rootloom-change-baseline-v2` and a contract skeleton before implementation. Analyzer-created v2 baselines include `run_id`, `nonce`, `task_sha256`, producer version, repository identity, Git identity, and sensitive-policy hash; v1 baselines remain readable as self-declared compatibility input. The baseline binds bounded Git/untracked state and metadata-only ignored/secret-like state so the finalizer can detect an intake-sensitive deletion that Git status cannot see. Ordinary untracked regular files receive streaming hashes and bounded text patches; binary/large files receive type, size, and hash. Sensitive files, directories, and symlinks are never content-read.

`rootloom-change-contract-v1` binds actual paths to allowed/forbidden globs, requires root-cause alignment for defect work, and maps primary/invariant/adjacent plus risk-specific claims to explicitly executed commands. Operator-sealed contracts also reference the baseline's hash, `run_id`, `nonce`, and `task_sha256`; structured claim bindings must name command IDs, target, evidence kind, and expected evidence. Commands declared only in the contract are not execution authority. The summary keeps `format: rootloom-engineering-summary-v1` with `schema_revision: 2`, retains `risk_assessment`, and separates command exit success, capture preservation, `claim_binding`, compatibility `verification_coverage`, `semantic_coverage`, provenance, hash chain, and `quality_status`; the compatibility `passed` field is true only for operator-sealed `VERIFIED_CHANGE`. Advisory mode may omit baseline/contract and default `--exit-policy bundle` exits successfully after stable passing commands while reporting `UNVERIFIED`; automation can use `--exit-policy quality` or `--require-verified`.

Verification runs in a controlled local process group or Windows Job Object and records `process_convergence` plus `isolation: process-group-only`. This is not a sandbox for untrusted commands and cannot guarantee control of detached services, containers, or privileged background managers.
