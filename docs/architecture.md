...
- a read-only Decision Pair verifier that reports `VALID`, `INVALID`, or `STALE` after checking Result binding, canonical Terminal/Summary agreement, decision-record hash, file identities, repository/protected-deletion commitment, and Run Directory identity;
