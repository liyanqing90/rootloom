...
- a Human Review v4 binding/decision protocol over the canonical final Result core and canonical run-directory identity, the complete final metadata-only floor, bounded worktree/index/Git-control commitments, exact-missing protected targets plus parent boundaries, and private Artifact hashes with count/per-file/aggregate/time budgets; Result is safely re-read after the terminal append and after recomputation, and any Result or state drift compensates the terminal record;
