...
- an optional absolute fingerprinted isolation-launcher argv prefix, with a required mode that fails before stages; containment remains owned by the launcher/platform;
- a Human Review v2 binding/decision protocol over the canonical final Result core, HEAD, status, private Artifact hashes captured through stable directory-relative no-follow descriptors, reviewer/local identity, no-follow terminal records, and acceptance-time drift checks;
- Manifest-bound apply/rollback setup journals with complete preflighted restoration plans, hash-aware orphan recovery, and ambiguous-edit refusal;
