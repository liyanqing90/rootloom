...
- 通过 selector 把合并输出流式写入有界 tail，并结构化记录观察/保留字节数、截断、输出超限、drain cutoff 与 detached 后代状态；
- 直接父进程退出但 stdout 仍打开时，立即清理原进程组并开始一秒本地 drain，不消耗剩余 stage timeout；
- Git Delta 以流式、完整或失败的方式生成私有产物，每次捕获默认总预算 32 MiB，普通未跟踪 patch 另有 8 MiB 默认预算；Runner 自有捕获共享 stage deadline/drain，禁用 external diff 与 textconv driver，补偿部分写入，并且只加载固定 Prompt 摘要；
- 每个模型阶段持久化结构化命令 sidecar，并为验证批次设置 32 MiB 保留输出总预算、带物化前转义计数和命令前最小记录预检的 64 MiB append-only NDJSON 预算、64 条命令上限和 120,000 字符紧凑验证 Prompt 上限；
- 确定性验证默认使用最小环境，额外既有变量只能按名称显式授权，元数据不记录变量值。

这里是流程确定性，不是结果确定性。JSON Schema 约束输出形状，本地语义门禁约束部分一致性；二者都不能证明证据真实、根因正确、所选验证命令充分或修改在生产环境安全。内置敏感名称识别是有限清单，仓库专有缺口应使用精确/递归自定义规则或可选 dotfile 脱敏。验证入口指纹消费仓库的 ignored/敏感分类，并在内容访问前拒绝 protected harness。它覆盖直接入口、常见入口和按命令绑定的操作方稳定性依赖，但不是能识别隐藏 CLI 路径的通用解析器，也不能证明命令在语义上真实使用了绑定依赖。pytest 位置参数会被视为选择范围，而不是可执行入口。每条验证命令前后都会检查入口和仓库状态，因此一条命令产生变更后，批次会在下一条命令运行前停止。产物脱敏不是文件访问隔离，因为每个模型阶段仍获得可读取仓库的 sandbox。已授权 protected 删除只能证明路径已移除，不能证明旧内容，因此不能自动验收；Rootloom 会把这类任务保持为 deletion-only，防止 protected 内容在同一次任务中通过 rename 或 move 混入普通 Delta。清理边界只覆盖原始 POSIX 进程组；创建新 session 的后代可以继续存活，但直接父进程完成会立即启动有界输出 drain，使其不能继续消耗剩余 stage timeout 或无限占用 Runner 与仓库锁。Delta 超限时要求完整内容并在 Review 前失败，不会把截断 patch 当成可审查内容。不可信命令需要容器、cgroup 或等价作业隔离。严格 Runner 支持 Linux、macOS 和 WSL；由于仓库锁与进程组终止依赖 POSIX 语义，原生 Windows 会被明确拒绝。
