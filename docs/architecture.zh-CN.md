...
事务只管理所选能力映射出的资产，再加一份分别控制两个生命周期 Hook 的私有组件策略。非阻塞跨进程锁会串行化同一 Codex home 的 setup 与 rollback。Setup、Guidance 与 Runner 仓库锁统一由 `plugins/rootloom/lib/rootloom_lock.py` 这一个加固协作式锁所有：它通过稳定的 no-follow/reparse-aware 父目录和目标句柄打开，拒绝非普通、多链接目标与身份替换，以非阻塞方式获取锁后才写入 owner 数据。这个 inode 锁不是不可替换租约或敌对进程互斥；同 UID 进程仍可 rename/unlink 已锁 inode、重建同路径，并独立锁住替代 inode。apply 与 rollback 都会在第一处目标修改前持久化被 Manifest 哈希绑定的阶段日志、完整操作前快照、Hash 与 Mode。Recovery 会在写入前验证完整、唯一路径且仅含托管目标的计划及全部恢复来源；状态提交属于同一补偿边界。`full` 事务包含完整全局 `AGENTS.md`、一个 profile、四个自定义 Agent 文件、一份 Rules，以及用户现有 `[agents]` 表中的三个限制键；其余 config 键全部保留。存在任一无托管冲突时，整个 apply 都会停止，除非用户明确授权替换。
...
审计深度由现有工作流承担，不再增加阶段：既有 Findings 只是线索；Evidence 检查同类实现并尝试证伪竞争假设；Diagnosis 用证据排除一个严肃备选；Review 从实际 Diff 出发，记录最强反例、已查同类实现和复杂度价值。审查结果区分已确认线索、新发现、已排除表面和未审表面。无法说清“会改变哪个决定”的机制不能通过价值门禁。

每条关键事实都需要稳定 ID、精确引用，以及 `repository` 或 `runtime_external` 来源类型。稳定的本地源码事实到此为止；运行时或外部证据还需记录环境、观察时间或时间窗、可用时的稳定 Artifact/查询/Trace/关联引用、新鲜度/脱敏，以及事实/推断状态。严格 Runner 中每条观察事实和复现记录都必须引用这些 ID。行为验证从被破坏的不变量推导，覆盖原始失败路径、所有者边界不变量和相邻负向或替代路径。
...
这条路径的长期资源与私有 Artifact 合同记录在 [在所属 Artifact 边界约束 Strict Runner 资源](decisions/2026-07-12-strict-runner-resource-artifacts.md)。
...
- 基线 protected 集合形成整个 Run 不可缩减的 metadata-only 下限；ignore 配置变化不能重新开放内容读取，去分类会在 Delta 捕获前被拒绝；Writer 对上述路径的任何变化默认同样在 Delta 捕获前被拒绝，唯一例外是提前精确授权的删除，而且最终必须以非零人工验收状态停止；
...
- 对已复现问题的竞争假设与反证、GO 前被排除备选、完成状态和挑战式 PASS 的结构化语义门禁；
- 在启动、每次 Writer 后、确定性验证后和最终 Review 后检查 topology，不在每个只读阶段重复完整遍历；
- 最多一次定向修复循环；
- 在超时、中断、父进程成功/失败退出但仍遗留子进程，或命令输出预算耗尽时终止原进程组，并以有界 SIGTERM 等待、SIGKILL 升级、进程组消失确认和最终输出 drain 硬期限完成清理；
- 通过 selector 把合并输出流式写入有界 tail，分别记录原始命令、Runner 有效及最终持久化退出码，并结构化记录观察/保留字节数、截断、输出超限、drain cutoff 与 detached 后代状态；
- 直接父进程退出但 stdout 仍打开时，立即清理原进程组并开始一秒本地 drain，不消耗剩余 stage timeout；drain 到期会令 Runner 有效结果变为 125，即使直接父进程返回 0，所有模型或确定性阶段也会关闭式失败；
- Git Delta 以流式、完整或失败的方式生成私有产物，每次捕获默认总预算 32 MiB，普通未跟踪 patch 另有 8 MiB 默认预算；Runner 自有捕获共享 stage deadline/drain，禁用 external diff 与 textconv driver，补齐合法 short write、拒绝非法进展、核对文件增长、回滚失败的整个未跟踪批次，并且只加载固定 Prompt 摘要；
- 每个模型阶段持久化结构化命令 sidecar，并为验证批次设置 32 MiB 保留输出总预算、带物化前转义计数、命令前最小记录预检和部分追加补偿的 64 MiB append-only 版本 2 NDJSON 预算、64 条命令上限和 120,000 字符紧凑验证 Prompt 上限；
- 为可见路径、status、Git index/控制数据与模型结构化输出设置生产者边界；锁定 Run 内仅在 device/inode/size/mode/mtime/ctime 不变时复用内容摘要；
- 提供位于仓库/run root 之外的可选绝对路径隔离启动器 argv 前缀、阶段前强制模式、稳定 no-follow 配置身份，并在每次 spawn 前重新核验实际身份；真实 containment 仍由启动器/平台拥有；
- 提供 Human Review v4 协议：绑定规范化最终 Result 核心与 Run 目录身份、完整最终验证 metadata-only 下限、精确最终检查点 worktree/index/Git-control 承诺、protected 目标 exact-missing 状态与父边界，以及在读取源头执行数量/单文件/合计/时间预算的私有 Artifact 哈希；每次 Result/Artifact 读取都会把规范名称绑定到所读描述符，Artifact 名称会在哈希前后重复枚举，而 pinned 普通单链接 Terminal 与 Summary 描述符组成同一个补偿事务，最终门禁会重新哈希精确 Payload，并检查规范名称、inode、单链接、mode、size 与 SHA-256；
- 提供带类型结果的只读 Decision Pair 验证器：完整 Result Envelope/Binding/Pair 无效为 `INVALID`/9，完成当前状态重捕获后的差异为 `STALE`/12，Git、权限/I/O、拓扑、Deadline 或本地消费者上限导致无法判断则为 `UNVERIFIED`/13；持久化预算不能抬高独立 verifier 上限，Git 重捕获关闭 Optional Lock、fsmonitor 与 untracked cache 写入，stdout 只含一个状态词，有界 stderr 携带原因；
- Apply/rollback setup 日志均被 Manifest 哈希绑定，恢复计划完整预检后才允许写入，并以哈希条件恢复 orphan transaction、拒绝含糊用户修改，同时使用独立于当前插件 catalog 的版本化历史 target schema；
- 确定性验证默认使用最小环境，额外既有变量只能按名称显式授权，元数据不记录变量值。

这里是流程确定性，不是结果确定性。JSON Schema 约束输出形状，本地语义门禁约束部分一致性；二者都不能证明证据真实、根因正确、所选验证命令充分或修改在生产环境安全。内置敏感名称识别是有限清单，仓库专有缺口应使用精确/递归自定义规则或可选 dotfile 脱敏。验证入口指纹消费仓库的 ignored/敏感分类，并在内容访问前拒绝 protected harness。它覆盖直接入口、常见入口和按命令绑定的操作方稳定性依赖，但不是能识别隐藏 CLI 路径的通用解析器，也不能证明命令在语义上真实使用了绑定依赖。pytest 位置参数会被视为选择范围，而不是可执行入口。每条验证命令前后都会检查入口和仓库状态，因此一条命令产生变更后，批次会在下一条命令运行前停止。产物脱敏不是文件访问隔离，因为每个模型阶段仍获得可读取仓库的 sandbox。已授权 protected 删除只能证明路径已移除，不能证明旧内容，因此不能自动验收；Rootloom 会把这类任务保持为 deletion-only，防止 protected 内容在同一次任务中通过 rename 或 move 混入普通 Delta。Human Review 是可信个人或小团队环境下的可归因本地终局，不是本地对抗型审批；只读快照、独立 Worker/UID、外部签名、远端不可变 Artifact Store 与 WORM 审计存储负责更高保证。清理边界只覆盖原始 POSIX 进程组；创建新 session 的后代可以继续存活，但检测到继承输出 drain 到期时会先以 125 关闭式失败，禁止任何自动验收。detached 进程仍可在失败后继续行动，也可通过提前关闭继承输出逃避该信号。Delta 超限时要求完整内容并在 Review 前失败，不会把截断 patch 当成可审查内容。不可信命令需要容器、cgroup 或等价作业隔离。严格 Runner 支持 Linux、macOS 和 WSL；由于仓库锁与进程组终止依赖 POSIX 语义，原生 Windows 会被明确拒绝。

生产关键工作仍需外部 OS/container、凭据、网络、分支保护和 CI 策略。

Human Review 组合职责拆分到 `scripts/human_review/`：`schema.py` 负责精确 Result Envelope 与持久化 Binding 校验，`binding.py` 负责当前状态预检/重算，`pinned_io.py` 负责通过稳定私有描述符读取 Pair，`decision.py` 负责统一身份/时间 Schema 与补偿式 Producer 事务，`verify.py` 负责类型化结果分类。中立的 `runner/errors.py`、`runner/contracts.py` 与 `runner/git_capture.py` 分别拥有共享异常、资源/状态合同和 verifier-only Git 行为。Human Review 通过显式 runtime 接口接收剩余 Runner 原语，不再导入 `run_pipeline.py`；顶层命令是组合与兼容边界。
