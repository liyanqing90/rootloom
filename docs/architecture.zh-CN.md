...
- 提供 Human Review v4 协议：绑定规范化最终 Result 核心与 Run 目录身份、完整最终 metadata-only 下限、受限 worktree/index/Git-control 承诺、protected 目标 exact-missing 状态与父边界，以及具有数量/单文件/合计/时间预算的私有 Artifact 哈希；终态追加后及重复承诺后都会安全重读 Result，任何 Result 或状态漂移都会补偿终态记录；
