...
事务只管理所选能力映射出的资产，再加一份分别控制两个生命周期 Hook 的私有组件策略。非阻塞跨进程锁会串行化同一 Codex home 的 setup 与 rollback；apply 与 rollback 都会在第一处目标修改前持久化被 Manifest 哈希绑定的阶段日志、完整操作前快照、Hash 与 Mode。Recovery 会在写入前验证完整、唯一路径且仅含托管目标的计划及全部恢复来源；状态提交属于同一补偿边界。`full` 事务包含完整全局 `AGENTS.md`、一个 profile、四个自定义 Agent 文件、一份 Rules，以及用户现有 `[agents]` 表中的三个限制键；其余 config 键全部保留。存在任一无托管冲突时，整个 apply 都会停止，除非用户明确授权替换。
...
- 提供 Human Review v3 协议：绑定规范化最终 Result 核心、受限 worktree/index/Git-control 承诺、protected 目标 exact-missing 状态与父边界、通过稳定目录相对 no-follow 描述符捕获的私有 Artifact 哈希、审核者/本地身份、仓库锁串行化、no-follow 终态记录和可补偿的写后漂移复查；
