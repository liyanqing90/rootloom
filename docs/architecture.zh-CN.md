...
- 提供 Human Review v4 协议：绑定规范化最终 Result 核心与 Run 目录身份、完整最终验证 metadata-only 下限、精确最终检查点 worktree/index/Git-control 承诺、protected 目标 exact-missing 状态与父边界，以及在读取源头执行数量/单文件/合计/时间预算的私有 Artifact 哈希；终态追加后及重复承诺后都会安全重读 Result，并由一个普通单链接 pinned descriptor 独占终态创建、追加、身份验证和补偿，任何 Result、终态身份或状态漂移都只补偿原始记录；
