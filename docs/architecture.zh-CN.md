...
- 提供只读 Decision Pair 验证器：在重新捕获前完整校验 v4 持久化 Schema，把格式错误、不安全或内部冲突证据报告为 `INVALID`/9，并只把结构有效但当前仓库、Artifact 或 protected deletion 状态不同的 Pair 报告为 `STALE`/12；stdout 只含一个状态词，有界 stderr 携带原因；
...
Human Review 组合职责拆分到 `scripts/human_review/`：`schema.py` 负责持久化校验，`binding.py` 负责当前状态重算，`pinned_io.py` 负责通过稳定私有描述符读取 Pair，`decision.py` 负责身份/Payload 预算和补偿式 Producer 事务，`verify.py` 负责 stale 分类。顶层 `review_decision.py` 只保留 CLI 解析、稳定状态映射、有界诊断与兼容导出。通用仓库捕获和底层描述符原语继续由 Runner 所有，避免模块拆分复制安全 I/O。
