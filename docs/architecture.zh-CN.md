...
- 提供可选的绝对路径、指纹化隔离启动器 argv 前缀及阶段前强制模式；真实 containment 仍由启动器/平台拥有；
- 提供绑定规范化最终 Result 核心、HEAD、status、通过稳定目录相对 no-follow 描述符捕获的私有 Artifact 哈希、审核者/本地身份、no-follow 终态记录和验收时漂移复查的 Human Review v2 协议；
- Apply/rollback setup 日志均被 Manifest 哈希绑定，恢复计划完整预检后才允许写入，并以哈希条件恢复 orphan transaction、拒绝含糊用户修改；
