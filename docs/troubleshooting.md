...
The Runner fingerprints detected verification entrypoints before the writer and checks them again before deterministic verification. It records common missing candidates too, so creating a new `GNUmakefile`, `pytest.ini`, or similar harness after baseline capture is treated as an entrypoint change. It also follows repository-internal symlink chains and binds the final target content.

If the writer modifies `Makefile`, `package.json`, pytest configuration, a detected repository-relative script used by `--verify`, or a path supplied with `--bind-verification-path`, the run stops before executing verification. Use an external trusted harness, pass important repo-owned harness files through `--bind-verification-path`, split test-harness changes into a separate task, or choose a verification command whose entrypoint is outside the writer's allowed scope.
