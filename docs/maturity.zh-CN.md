...
严格高保障 Runner 支持 Linux、macOS 和 WSL，并拒绝原生 Windows。v1.2.15 CI 矩阵在 Python 3.11–3.14 上运行完整 Linux 检查、macOS Strict Runner 测试、macOS/Windows 可移植 setup/guidance/hook 与共享锁测试，以及固定 Codex CLI compatibility smoke；由于 Windows 暴露的是 ACL/只读语义而不是 `fchmod`，精确 POSIX 权限模式断言仍只在 Linux/macOS 执行。发布运行 `29227221911` 的八个 Job 已针对精确标签提交 `2fdffb6f9b6814ca945182c35abbcec4c6d9c2a6` 全部通过。当前已验证 spawn 表面仍不能证明原生自定义角色/模型路由，因此必须给出模型分配证据时，应使用连续 Runner。
