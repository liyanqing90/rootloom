...
`HUMAN_REVIEW_REQUIRED` 现在拥有 v4 accept/reject 命令：绑定受限的规范化 worktree/index/Git-control 承诺、protected 目标 exact-missing 状态与父边界、规范化 Run 目录身份、完整最终 metadata-only 下限、受限私有 Artifact 哈希，以及审核者/本地身份。Result 会在追加前、追加后及重复承诺后通过稳定 no-follow 描述符读取；复制 Run、Result 修改、仓库漂移或 protected 路径去分类都会拒绝或补偿决定。单文件/合计 Artifact 字节、文件数、绑定时间及 Result 大小都有边界。决定记录在私有 Run 目录中追加写入，并拒绝重复终局决定。协作式锁与重复检查仍不能阻止任意外部 Writer；记录也不是组织身份密码学签名或存储级不可变记录，生产治理仍可要求不可变快照、外部签名者或 WORM 审计存储。
