...
| 声明 | 当前证据 | 不保证 |
| --- | --- | --- |
| Setup 会补偿可捕获失败，并恢复已记录的 apply/rollback 中断 | 跨进程锁、Manifest 绑定恢复日志、状态提交故障注入补偿、模拟 apply/rollback 突然中断后执行 `recover`、哈希和权限模式保留回滚测试 | 真实 `SIGKILL`、主机/断电故障、存储损坏，或原子文件替换之下的父目录持久性 |
...
`HUMAN_REVIEW_REQUIRED` 现在拥有 v3 accept/reject 命令：在协作式仓库锁内绑定受限的规范化 worktree/index/Git-control 承诺、protected 目标 exact-missing 状态与父边界、私有 Artifact 哈希、审核者/本地身份，并在写入前后复查漂移。重新创建 ignored 删除目标会拒绝决定；检测到写入窗口漂移会补偿刚写入的记录。决定记录在私有 Run 目录中追加写入，并拒绝重复终局决定。仓库锁不能阻止任意外部 Writer；记录也不是组织身份密码学签名或存储级不可变记录，生产治理仍可要求不可变快照、外部签名者或 WORM 审计存储。
...
严格高保障 Runner 支持 Linux、macOS 和 WSL，并拒绝原生 Windows。v1.2.13 CI 矩阵在 Python 3.11–3.14 上运行完整 Linux 检查、macOS Strict Runner 测试、macOS/Windows 可移植 setup/guidance/hook 测试，以及固定 Codex CLI compatibility smoke；由于 Windows 暴露的是 ACL/只读语义而不是 `fchmod`，精确 POSIX 权限模式断言仍只在 Linux/macOS 执行。发布运行 `29218683045` 的八个 Job 已针对精确标签提交 `05627be11d17ff9aea2cd6997c73549eb5874f52` 全部通过。当前已验证 spawn 表面仍不能证明原生自定义角色/模型路由，因此必须给出模型分配证据时，应使用连续 Runner。
