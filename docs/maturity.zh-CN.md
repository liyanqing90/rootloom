...
`HUMAN_REVIEW_REQUIRED` 现在拥有绑定 HEAD、Git status、私有 Artifact 哈希、审核者/本地身份和验收时漂移复查的 accept/reject 命令。决定记录在私有 Run 目录中追加写入，并拒绝重复终局决定。它不是组织身份的密码学签名，也不是存储级不可变记录；生产治理仍可要求外部签名者或 WORM 审计存储。
...
严格高保障 Runner 支持 Linux、macOS 和 WSL，并拒绝原生 Windows。候选 CI 矩阵会运行完整 Linux 检查、macOS Strict Runner 测试，以及 macOS/Windows 的可移植 setup/guidance/hook 测试；由于 Windows 暴露的是 ACL/只读语义而不是 `fchmod`，精确 POSIX 权限模式断言仍只在 Linux/macOS 执行。只有精确候选推送并观察到这些新 Job 通过后，它们才构成认证证据。当前已验证 spawn 表面仍不能证明原生自定义角色/模型路由，因此必须给出模型分配证据时，应使用连续 Runner。
