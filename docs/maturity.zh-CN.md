...
脱敏跟踪路径，而不跟踪字节来源。把 protected 字节复制到普通允许路径后，副本可能进入内容哈希和 Delta。Delta 生成会在显式 tracked/普通未跟踪字节预算下流式执行并要求完整或失败；内部捕获会禁用仓库 diff/textconv 执行并只加载固定 Prompt 摘要，但它仍不是内容血缘 DLP。进程清理也只治理原始 POSIX 进程组：后代可以创建新 session 并继续存活。直接父进程完成时，托管命令和 Runner 自有 Artifact 捕获都会立即启动本地 drain 硬期限，避免逃逸描述符持有者继续消耗剩余 stage timeout 或无限占用仓库锁，但不可信命令仍需要容器、cgroup 或等价作业隔离。
