...
Rootloom Personal Core 2.1 是早期、单维护者产品。目标是让 Codex 工程行为更审慎、更可检查；仓库目前还没有受控证据证明它能降低缺陷或审查时间。
...
- 拒绝漂移的备份恢复；
- 不使用 shell、有界输出的验证命令；
- 仓库外 review bundle，以及有界 patch、命令数量与聚合日志；
- 敏感删除路径精确确认；
- 综合任务、路径、tracked diff、操作与活跃记忆信号的可解释静态风险下限；
- 与已执行测试证据分离的风险专属验证建议；
- 有界、相关性检索、过期感知的项目记忆上下文与经过 Schema 检查的显式更新；
