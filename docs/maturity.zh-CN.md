...
| 高保障执行具有确定性 | 固定阶段顺序、已关联来源 ID、ignored/敏感内容元数据化、捕获前范围门禁、映射命令结果、检测到和操作方绑定的验证入口指纹与有限修复循环 | 模型证据真实、隐藏 CLI 路径解析、完整依赖闭包绑定、所选命令语义充分、根因正确、业务正确或生产安全 |
...
检测到的 metadata-only 路径变化默认没有资格获得自动验收。门禁发生在 Writer 之后，不是 OS 级写入预防或回滚，因此失败后操作方可能需要恢复 protected 文件系统变化。操作方可以精确授权删除一个基线既有路径，但授权会在 Writer 前预检，要求干净基线和无修复循环，并强制该任务成为 deletion-only；普通修改、rename、move 或 visible 文件创建必须拆到另一次任务。Runner 随后会以 `HUMAN_REVIEW_REQUIRED` 退出；由于模型与产物都不能证明旧的 protected 内容，它不能声称自动 PASS。Topology 检查点降低了 Writer 或验证命令新建嵌套仓库的 TOCTOU 风险，检测到和操作方绑定的验证入口指纹降低了 Writer 重定向 harness 的风险，但任意外部进程、隐藏命令间接引用和未绑定的 harness 依赖文件仍可能在这些检查点外改变行为。
