...
`HUMAN_REVIEW_REQUIRED` 现在拥有 v4 accept/reject 命令：绑定精确的最终验证 worktree/index/Git-control 承诺、protected 目标 exact-missing 状态与父边界、规范化 Run 目录身份、完整最终 metadata-only 下限、受限私有 Artifact 哈希，以及审核者/本地身份。初次 Binding 会在读取前沿用已验证下限，并拒绝承诺漂移而不是吸收它。Result 会在追加前、追加后及重复承诺后通过稳定 no-follow 描述符读取；复制 Run、Result 修改、仓库漂移或 protected 路径去分类都会拒绝或补偿决定。单文件/合计 Artifact 字节由描述符实际读取量在源头执行，文件数、绑定时间及 Result 大小另有边界。Terminal 与 Summary 共用 Producer/Consumer 各 1 MiB 上限，在任一文件存在前完成预检，并保持 pinned 直到最终精确字节数/SHA-256 校验；reviewer/local-account 身份限制为 4096 个 UTF-8 字节。验证器会在重捕获前完整校验持久化 Binding Schema 与全部内部承诺，把格式错误证据固定为 `INVALID`/9，并只把结构有效的当前状态漂移归为 `STALE`/12；有界 stderr 给出原因，stdout 保持机器稳定。Schema、Binding 重算、Pair I/O、决定事务与验证分类现已分属独立模块。加固协作式锁与重复检查仍不能阻止任意同 UID Writer，也不能阻止已锁路径被新 inode 替换。因此 Human Review v4 到此冻结为可信个人或小团队环境下的可归因人工终局，而不是本地对抗型审批或带密码学签名/WORM 的组织证据；更强治理应由不可变快照、独立 Worker 或 UID、外部签名者、远端不可变 Artifact Store 与 WORM 审计系统承担。
...
严格高保障 Runner 支持 Linux、macOS 和 WSL，并拒绝原生 Windows。v1.2.17 CI 矩阵在 Python 3.11–3.14 上运行完整 Linux 检查、macOS Strict Runner 测试、macOS/Windows 可移植 setup/guidance/hook 与加固协作式锁测试，以及固定 Codex CLI compatibility smoke；由于 Windows 暴露的是 ACL/只读语义而不是 `fchmod`，精确 POSIX 权限模式断言仍只在 Linux/macOS 执行。发布运行 `29234377961` 的八个 Job 已针对精确标签提交 `4b749632ca042a73546defd1e3ff0ddf2bdfe82c` 全部通过。当前已验证 spawn 表面仍不能证明原生自定义角色/模型路由，因此必须给出模型分配证据时，应使用连续 Runner。
