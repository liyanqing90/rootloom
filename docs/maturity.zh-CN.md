...
| 高保障执行具有确定性 | 固定阶段顺序、已关联来源 ID、ignored/敏感内容元数据化、捕获前范围门禁、映射命令结果与有限修复循环 | 模型证据真实、所选命令语义充分、根因正确、业务正确或生产安全 |
...
检测到的 metadata-only 路径变化默认没有资格获得自动验收。门禁发生在 Writer 之后，不是 OS 级写入预防或回滚，因此失败后操作方可能需要恢复 protected 文件系统变化。操作方可以精确授权删除一个基线既有路径，但 Runner 随后会以 `HUMAN_REVIEW_REQUIRED` 退出；由于模型与产物都不能证明旧的 protected 内容，它不能声称自动 PASS。Topology 检查点降低了 Writer 新建嵌套仓库的 TOCTOU 风险，但任意外部进程仍可能在检查点之间修改仓库。
