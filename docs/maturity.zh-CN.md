...
检测到的 metadata-only 路径变化默认没有资格获得自动验收。门禁发生在 Writer 之后，不是 OS 级写入预防或回滚，因此失败后操作方可能需要恢复 protected 文件系统变化。操作方可以精确授权删除一个基线既有路径，但授权会在 Writer 前预检，并强制该任务成为 deletion-only；普通修改、rename、move 或 visible 文件创建必须拆到另一次任务。Runner 随后会以 `HUMAN_REVIEW_REQUIRED` 退出；由于模型与产物都不能证明旧的 protected 内容，它不能声称自动 PASS。Topology 检查点降低了 Writer 或验证命令新建嵌套仓库的 TOCTOU 风险，但任意外部进程仍可能在检查点之间修改仓库。
