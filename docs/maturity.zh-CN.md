...
严格高保障 Runner 支持 Linux、macOS 和 WSL，并拒绝原生 Windows。候选 CI 矩阵会运行完整 Linux 检查、macOS Strict Runner 测试，以及 macOS/Windows 的可移植 setup/guidance/hook 测试；只有精确候选推送并观察到这些新 Job 通过后，它们才构成认证证据。当前已验证 spawn 表面仍不能证明原生自定义角色/模型路由，因此必须给出模型分配证据时，应使用连续 Runner。
