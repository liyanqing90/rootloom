...
| 声明 | 当前证据 | 不保证 |
| --- | --- | --- |
| Setup 具备事务性 | 跨进程锁、预备恢复清单、状态提交故障注入补偿、哈希和权限模式保留回滚测试 | 受管 Codex home 之外的失败，或忽略锁的其他软件同时修改 |
| 项目播种受限 | 确定性本地扫描器、仓库内非符号链接证据、Git 公共目录写锁、快照复查与回归测试 | 语义完整性或始终准确的架构认知 |
| 命令策略有明确意图 | 固定版与最新版 CLI 的离线生命周期冒烟测试，覆盖 commit、push 和破坏性 reset | 所有管理员策略、平台策略或未来 CLI 的组合结果 |
| 高保障执行具有确定性 | 固定阶段顺序、已关联来源 ID、仓库状态门禁、允许路径、命令结果与有限修复循环 | 模型证据真实、根因正确、业务正确或生产安全 |
...
两条轨道都会运行离线生命周期冒烟：本地 marketplace 注册、插件安装/发现、完整 setup 与 status、setup 验证、高保障 profile 解析、commit/push/reset Rules 决策、完整回滚以及既有配置保留。它无需凭据和模型调用，用来验证集成形状；不证明未来交互式 Hook 语义或模型行为。
