...
默认 32 KiB 的项目指令预算是上限，不是目标。本项目的全局托管模板控制在 16 KiB 内，自动生成的项目指导更短。
...
托管区块下方的用户区只保留持久且有路径证据的不变量，例如所有权方向、生成代码边界、公开或持久化契约，以及权威架构或迁移文档。可选 `.project-memory/` 用可审查的风险和失败经验补充指导，但不能替代 `AGENTS.md` 权威或当前仓库证据。
