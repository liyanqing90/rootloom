...
SessionStart Hook 只把可安全再生的事实临时注入当前会话，不写入仓库：
...
只有显式调用 `$seed-project-guidance` 才会持久化托管区块。其下方用户区只保留持久且有路径证据的不变量，例如所有权方向、生成代码边界、公开或持久化契约，以及权威架构或迁移文档。Experimental `.project-memory/` 可用可审查的风险和失败经验补充指导，但不能替代 `AGENTS.md` 权威或当前仓库证据。
