...
严格 Tier 1/2 发布或显式治理审查应在实现前通过 `begin_review.py --repo ... --task ... --output ...` 创建 intake，或用 `analyze_change.py --write-baseline` 创建 baseline 并提供匹配 contract，然后用 `--strict` 运行 finalizer。Baseline v2、contract 与 finalizer summary 会记录 `run_id`、`nonce`、`task_sha256` 和 hash 链，使 operator-sealed 证据能与 self-declared 证据区分开。外部输出目录必须不存在、为空或已经由 Rootloom 标记拥有。工具不使用 shell。

普通 untracked 文件使用流式 SHA-256 指纹与有界文本 patch；二进制/大文件记录类型、大小与 Hash。ignored、secret-like、用户声明敏感、目录和 symlink 路径只记录元数据。Git/status/patch 与命令输出在读取期间即受限；超时、输出超限或残留子进程会终止整个受控进程树。Summary 分开表达 `commands_passed`、`capture_preserved`、`claim_binding`、兼容字段 `verification_coverage` 与 `semantic_coverage`；只有 operator-sealed 的完整证据才得到 `quality_status: VERIFIED_CHANGE`。Personal Core 会报告 `isolation: process-group-only`；它不是执行不可信命令的沙箱。纯验证必须使用 `--allow-no-change`，结果为 `NO_CHANGE`。受保护删除仍要求精确 `--confirm-dangerous-delete` 授权。
