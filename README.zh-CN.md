...
秘密材料发现会把共享的大小写不敏感候选 Pathspec 与用户声明的 Literal Root 交给 Git，而不是枚举全部 Tracked/Ignored 路径。Python 使用 `is_sensitive_material_path()` 再次分类全部候选项；独立的有界候选上限保护定向 Git 查询，`--max-sensitive-paths`（默认 10000）只统计分类后的材料结果并集，超限即失败。材料包括 `.env*`、Credential 配置、Private Key、Certificate、Keystore、显式 Root，以及 `clientSecret.json`、`apiToken.json`、`serviceAccountKey.json`、`apiKey.json` 等 CamelCase 名称。`src/auth/token.py`、`src/credential_store.ts`、`internal/secret_manager.go` 等安全领域源码由 `is_security_domain_path()` 处理：它们会提高风险与验证要求，但保留可审查 Patch，也不触发隐私删除确认。材料内容始终不读取；Symlink Target 只记录字节长度与 SHA-256。只要观察到材料元数据变化——包括相对 Baseline 或验证前 Capture 新发现的 Ignored 新增——就会在普通内容捕获前进入隔离：所有变更端点只记录元数据，停止额外的 Repository Memory/Command Discovery，并把被忽略的材料新增、修改和删除合成为 Contract Scope 内的显式变更。Summary 明确写为 `sensitive_integrity: metadata-observed`，不宣称内容完整性。
