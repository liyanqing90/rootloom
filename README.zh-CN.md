...
它定义授权、可逆自主执行、工作区保护、根因与范围默认值、证据标准、流程路由、自动项目指导、委派上限和精简沟通；不包含仓库命令、框架偏好、项目架构或人格扮演。
...
| `$operating-coding-change` | 普通 R1/R2 实施 | 根因、小范围 diff、成比例验证 |
| `$operating-code-review` | 只读审查 | 证据化 findings 优先，不修改 |
| `$operating-high-risk-change` | API、Schema、数据、安全、基础设施、部署、发布 | ExecPlan、兼容、回滚、授权门禁 |
