...
每批确定性验证还拥有 32 MiB 保留输出、64 MiB 实际序列化 NDJSON 和 64 条命令三重上限。`--max-verification-output-bytes` 与 `--max-verification-artifact-bytes` 可以调整正整数字节预算；命令数量上限固定。记录只向 `*-verification.ndjson` 追加一次，并生成带版本的小型 `*-verification-summary.json` 索引，统计观察字节、保留字节、UTF-8 字节、JSON 记录字节和累计产物字节。Runner 会先计算 JSON 转义字节，再决定是否物化被拒绝的输出；每条命令运行前也会预检关闭式失败记录的最小空间。若产物预算耗尽，输出会标记为截断且验证关闭式失败。提供给修复与 Review 阶段的紧凑验证摘要仍最多 120,000 字符。

Git Delta 也从产生源头受限。staged、unstaged、`HEAD` 到 worktree 以及普通未跟踪 binary patch 会直接流式写入私有产物，并且每次捕获必须完整容纳在默认 32 MiB 总预算内；其中未跟踪 patch 另有 8 MiB 默认上限。一次捕获内的全部 Git 命令共享同一个 stage deadline 和父退出即时 drain，禁用 external diff 与 textconv driver，并在任何失败后把产物恢复到命令前长度。完整 patch 保留在磁盘，模型侧状态对每种 patch 视图最多读取 24,000 原始字节并附加标记，避免 decode/JSON 放大。只有评估内存、磁盘和审查成本后，才应调整 `--max-delta-bytes` 或 `--max-untracked-patch-bytes`。任何不完整捕获都会在自动 Review 前停止；被截断的源内容不能获得 PASS。

直接父进程退出但 stdout 仍未关闭时，Runner 会立即清理原进程组并开始一秒本地 drain，不再等待剩余 stage timeout。若描述符持有者已逃逸到新 session，则保留直接父进程退出码，同时用结构化 drain cutoff 与 detached 后代字段记录剩余风险。这些是本地 Runner 边界，不是对创建新 session 后代的宿主级隔离。
