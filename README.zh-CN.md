...
Protected 删除产生的退出 10 可通过 `review_decision.py --repo ... --run-dir ... --reviewer ... --decision accept|reject` 处理。Human Review v4 会绑定规范化 Run 路径与目录身份，要求 Result 的 `run_dir` 与之相符，并在任何内容指纹前把完整的最终验证 metadata-only 下限传入每次捕获；初次 Binding 必须与该最终验证仓库承诺精确一致，而不能吸收稍后的状态。它重新核对规范化最终 Result 核心、受限的可见内容与 metadata-only 承诺、status/路径集合、Git index/control 状态、每个 protected 目标的 exact-missing 状态与词法父边界，以及私有 Artifact 哈希。Artifact 哈希默认限制为单文件 64 MiB、合计 512 MiB、256 个文件和 120 秒；实际观察字节会在读取源头即时消耗单文件及剩余合计额度。每次 Result 与评审 Artifact 读取都会同时证明描述符元数据稳定，且规范目录项仍指向该描述符；Artifact 枚举还会在哈希前后承诺完全一致的名称集合。终态 NDJSON 与 Summary JSON 都必须是 pinned Run Directory 下的普通单链接文件，并在追加、fsync、最终校验和补偿期间保持 pinned descriptor。提交门禁会通过这些固定描述符重新读取精确预期字节数，并要求规范名称、inode、单链接、mode、size 与 SHA-256 全部等于 Rootloom 的原始 Payload。任一失败都会先把两个 pinned 原始 inode 截断为空，使重试没有歧义。

可用 `review_decision.py verify --repo ... --run-dir ...` 对既有 Decision Pair 做只读审计。stdout 精确保持为 `VALID`（退出 0）、`INVALID`（退出 9）或 `STALE`（退出 12）；INVALID 与 STALE 还会在 stderr 输出一条有界原因，但绝不序列化 Artifact 字节。在读取任何仓库或已审 Artifact 之前，验证器会完整校验 v4 Binding 的精确字段集、仓库与 Run Directory 身份、Git/Result Hash、规范 state policy 与路径列表、Repository Commitment 格式和 Hash、Artifact Map 与持久化预算、protected deletion commitment，以及 Binding/Result 一致性；随后才核对唯一规范 Terminal、Summary 中 reviewer/decision/binding 与 `decision_record_sha256` 的一致性和普通单链接文件。只有结构已确认有效、但当前仓库、Artifact 或 protected 状态发生变化的证据才是 `STALE`；任何格式错误、不安全、非规范或内部冲突都固定为 `INVALID`/9。

Producer 与 Consumer 对 Terminal 和 Summary 各自共用 1 MiB 上限；两份 Payload 都会在创建任一路径前完成预检，reviewer 与 local account 身份限制为 4096 个 UTF-8 字节，所有已提交 Pair 都必须能被当前验证器立即读取。既有 v2/v3 结果与旧版非空 Terminal 但缺少 Summary 的状态继续关闭式失败。Human Review v4 仍冻结为可信个人或小团队环境下的可归因人工终局；它不是本地对抗型审批、组织身份密码学证明或 WORM 存储。更高保证应由只读快照、独立 Worker 或 UID、外部签名者、远端不可变 Artifact Store 与 WORM 审计基础设施承担。
