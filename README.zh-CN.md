...
秘密材料发现会把共享的大小写不敏感候选 Pathspec 与用户声明的 Literal Root 交给 Git，而不是枚举全部 Tracked/Ignored 路径。Python 使用 `is_sensitive_material_path()` 再次分类全部候选项；独立的有界候选上限保护定向 Git 查询，`--max-sensitive-paths`（默认 10000）只统计分类后的材料结果并集，超限即失败。材料包括 `.env`、`.envrc`、非模板 `.env.<name>`、Credential 配置、Private Key/Keystore 格式、歧义 `.pem` 与 `.der`、显式 Root、常见 `key.pem` / `server-key.pem` 形式，以及 `clientSecret.json`、`apiToken.json`、`serviceAccountKey.json`、`apiKey.json` 等 CamelCase 名称。`.env.example`、`.env.sample`、`.env.template`、`.env.dist` 与公共证书格式（`.crt`、`.cer`、`.p7b`、`.p7c`）保留可审查 Patch，但会提高安全风险；DER 只是编码方式，不能证明内容是证书，因此默认只保留元数据，只有 Intake 显式声明后才可审查。`.environment`、`.envelope` 与 `.envoy` 是普通名称。`src/auth/token.py`、`src/credential_store.ts`、`internal/secret_manager.go` 等安全领域源码同样由 `is_security_domain_path()` 处理：它们会提高风险与验证要求，但不触发隐私删除确认。材料内容始终不读取；Symlink Target 只记录字节长度与 SHA-256。只要观察到材料元数据变化——包括相对 Baseline 或验证前 Capture 新发现的 Ignored 新增——就会在普通内容捕获前进入隔离：所有变更端点只记录元数据，停止额外的 Repository Memory/Command Discovery，并把被忽略的材料新增、修改和删除合成为 Contract Scope 内的显式变更。Summary 明确写为 `sensitive_integrity: metadata-observed`，不宣称内容完整性。

Evidence JSON 会拒绝重复 Key、非标准常量和超范围数值。所有验证命令必须在第一条执行前完成解析；Git/Status/Patch 捕获与聚合命令输出在读取期间即受限，超时、输出超限或残留子进程会终止受控进程树。Summary Revision 5 分开表达实际执行命令、Capture 保持、来自 Sealed Contract 的结构化 Claim Binding、一般声明、`semantic_coverage` 与顶层 `semantic_review`。新增的 `reviewability_policy` 对象会披露是否启用 Intake-sealed 例外、精确路径、Policy Hash、来源以及最终 Capture 的文件身份与 Link Count 元数据。`semantic_coverage: reviewed` 只会产生明确的 `semantic_review: operator-asserted`，并不是机器证明；语义覆盖未知时最高只能得到 `MECHANICALLY_VERIFIED`，没有封存链的语义声明是 `SEMANTIC_REVIEW_ASSERTED`。Workflow-sealed 机械证据加上该断言会得到 `REVIEW_EVIDENCE_COMPLETE`，它只表示审查证据链完整，不表示 Rootloom 已证明修改正确。`evidence_complete` 是稳定的自动化能力字段，`quality_status` 保留详细诊断；Provenance 使用不暗示身份保证的 `intake-sealed` 与 `workflow-sealed`。任何原本可完成但发生材料隔离的审查最高只能得到 `REVIEW_REQUIRED_WITH_REDACTIONS`、`evidence_complete: false` 与 `passed: false`。Strict 默认采用 Quality Exit，只有 `REVIEW_EVIDENCE_COMPLETE` 返回 0；需要非阻断 Bundle 时必须显式使用 `--strict-bundle-only`。Revision 4 Consumer 必须按 `schema_revision` 处理新的 Provenance 值；历史 Revision 3 Consumer 仍需从 `VERIFIED_CHANGE` 迁移到 `REVIEW_EVIDENCE_COMPLETE`。纯验证必须使用 `--allow-no-change`，但 Gate Error 优先于 `NO_CHANGE`。受保护删除覆盖秘密材料、Migration 与 Database Artifact，并要求精确 `--confirm-dangerous-delete` 授权。
