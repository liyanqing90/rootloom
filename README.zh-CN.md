...
可用 `review_decision.py verify --repo ... --run-dir ...` 对既有 Decision Pair 做只读审计。它只输出 `VALID`（退出 0）、`INVALID`（退出 9）或 `STALE`（退出 12）。验证器会核对 Result Binding、唯一规范 Terminal 记录、Summary 中 reviewer/decision/binding 与 `decision_record_sha256` 的规范一致性、普通单链接文件、当前仓库/protected deletion 承诺及 Run Directory 身份。既有 v2/v3 结果与旧版非空 Terminal 但缺少 Summary 的状态继续关闭式失败。Human Review v4 到此冻结为可信个人或小团队环境下的可归因人工终局；它不是本地对抗型审批、组织身份密码学证明或 WORM 存储。更高保证应由只读快照、独立 Worker 或 UID、外部签名者、远端不可变 Artifact Store 与 WORM 审计基础设施承担。
