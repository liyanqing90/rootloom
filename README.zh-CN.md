...
Protected 删除产生的退出 10 可通过 `review_decision.py --repo ... --run-dir ... --reviewer ... --decision accept|reject` 处理。Human Review v4 会绑定规范化 Run 路径与目录身份，要求 Result 的 `run_dir` 与之相符，并在任何内容指纹前把完整的最终验证 metadata-only 下限传入每次捕获；初次 Binding 必须与该最终验证仓库承诺精确一致，而不能吸收稍后的状态。它重新核对规范化最终 Result 核心、受限的可见内容与 metadata-only 承诺、status/路径集合、Git index/control 状态、每个 protected 目标的 exact-missing 状态与词法父边界，以及私有 Artifact 哈希。Artifact 哈希默认限制为单文件 64 MiB、合计 512 MiB、256 个文件和 120 秒；实际观察字节会在读取源头即时消耗单文件及剩余合计额度，其中三个可配置边界见上方命令。Result 会在决定前、追加终态后和重复状态承诺后，通过受限的目录相对 no-follow 描述符读取。终态 NDJSON 必须是普通单链接文件，并在追加、fsync、最终身份验证和任何补偿期间始终使用同一个目录相对 pinned descriptor。完整规范化 Result、终态身份或仓库发生漂移都只会补偿该 pinned 记录。既有 v2/v3 结果会关闭式失败，不会静默升级。仓库锁只能串行化协作式 Rootloom Writer，因此这仍是可归因本地审批，不是组织身份密码学证明、WORM 存储，也不能阻止最终检查后的任意外部进程。
