...
只有连续两次有界采集在 Snapshot、Tracked/Untracked Patch、HEAD/Ref 与 Index 上完全一致，Repository State 才会被接受。每个 Git 子进程都复用验证命令的受控进程树所有者、关闭标准输入，并受有限正数的 `--max-git-seconds` 限制（默认 30 秒）。普通 Untracked 文件使用流式 SHA-256 指纹与有界、可应用的文本 Patch；其中归属于任务的非敏感文本也进入风险分析。二进制/大文件记录类型、大小与 Hash。敏感发现会把共享的大小写不敏感内置 Pathspec 与用户声明的 Literal Root 交给 Git，而不是枚举全部 Tracked/Ignored 路径。Python 会再次分类全部候选项；独立的有界候选上限保护定向 Git 查询，`--max-sensitive-paths`（默认 10000）只统计分类后的敏感结果并集，超限即失败。这既避免全量枚举 Vendor/Cache，又不会静默排除其中匹配敏感规则的路径。内置及用户声明的敏感目录祖先会递归保护全部后代。敏感内容始终不读取；Symlink Target 只记录字节长度与 SHA-256。只要观察到敏感元数据变化——包括相对 Baseline 或验证前 Capture 新发现的 Ignored 敏感新增——就会在普通内容捕获前进入隔离：所有变更端点只记录元数据，停止额外的 Repository Memory/Command Discovery，并把被忽略的敏感新增、修改和删除合成为 Contract Scope 内的显式变更。Summary 明确写为 `sensitive_integrity: metadata-observed`，不宣称内容完整性。

Evidence JSON 会拒绝重复 Key、非标准常量和超范围数值。所有验证命令必须在第一条执行前完成解析；Git/Status/Patch 捕获与聚合命令输出在读取期间即受限，超时、输出超限或残留子进程会终止受控进程树。Summary Revision 4 分开表达实际执行命令、Capture 保持、来自 Sealed Contract 的结构化 Claim Binding、一般声明、`semantic_coverage` 与顶层 `semantic_review`。`semantic_coverage: reviewed` 只会产生明确的 `semantic_review: operator-asserted`，并不是机器证明；语义覆盖未知时最高只能得到 `MECHANICALLY_VERIFIED`，没有封存链的语义声明是 `SEMANTIC_REVIEW_ASSERTED`。封存完整的机械证据加上该断言会得到 `REVIEW_EVIDENCE_COMPLETE`，它只表示审查证据链完整，不表示 Rootloom 已证明修改正确。任何原本可完成但发生敏感隔离的审查最高只能得到 `REVIEW_REQUIRED_WITH_REDACTIONS` 与 `passed: false`。Strict 默认采用 Quality Exit，只有 `REVIEW_EVIDENCE_COMPLETE` 返回 0；需要非阻断 Bundle 时必须显式使用 `--strict-bundle-only`。Revision 3 消费者在 `schema_revision >= 4` 时应把精确 `VERIFIED_CHANGE` 判断迁移为 `REVIEW_EVIDENCE_COMPLETE`。纯验证必须使用 `--allow-no-change`，但 Gate Error 优先于 `NO_CHANGE`。受保护删除覆盖 Secret-like、Migration 与 Database Artifact，并要求精确 `--confirm-dangerous-delete` 授权。
