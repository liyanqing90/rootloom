...
用户报告和既有评审只是线索，不是审计边界。审查从失败表面出发，映射所有者与消费者，检查一个相邻同类实现，并尝试最强反例；结果区分已确认线索、新发现、已排除表面和未审表面。新 Guard、Flag、Journal、抽象或流程步骤必须说清失败、所有者、可执行约束、回归证明以及它会改变哪个决定，否则默认删除或改用更小的原生控制。Rootloom 复用现有 Evidence、Diagnosis、Review 字段完成挑战，不新增阶段或常驻审计 Agent。
...
| `$operating-code-review` | 只读审查 | Findings 加发现产出、反例、同类实现和机制价值检查；不修改 |
| `$operating-high-risk-change` | Tier 2 Governed 的 API、Schema、数据、安全、基础设施、部署、发布或不确定根因 | 治理任务包、会改变决定的门禁、新鲜挑战、兼容与回滚 |
| `$high-assurance-coding-change` | 用户显式要求受控多代理 | 证据证伪 → 根因门禁 → 唯一写代理 → 确定性验证 → 挑战式评审 |
...
每批确定性验证还拥有 32 MiB 保留输出、64 MiB 实际序列化 NDJSON 和 64 条命令三重上限。`--max-verification-output-bytes` 与 `--max-verification-artifact-bytes` 可以调整正整数字节预算；命令数量上限固定。版本 2 记录只向 `*-verification.ndjson` 追加一次，并生成带版本的小型 `*-verification-summary.json` 索引，统计观察字节、保留字节、UTF-8 字节、JSON 记录字节和累计产物字节。每条记录分别保留直接进程的 `command_exit_code`、生命周期治理后的 `runner_exit_code`，以及产物截断强制失败时的最终 `exit_code`。Runner 会先计算 JSON 转义字节，再决定是否物化被拒绝的输出；每条命令运行前也会预检关闭式失败记录的最小空间，磁盘部分追加会回滚。若产物预算耗尽，输出会标记为截断且验证关闭式失败。提供给修复与 Review 阶段的紧凑验证摘要仍最多 120,000 字符。

Git Delta 也从产生源头受限。staged、unstaged、`HEAD` 到 worktree 以及普通未跟踪 binary patch 会直接流式写入私有产物，并且每次捕获必须完整容纳在默认 32 MiB 总预算内；其中未跟踪 patch 另有 8 MiB 默认上限。一次捕获内的全部 Git 命令共享同一个 stage deadline 和父退出即时 drain，禁用 external diff 与 textconv driver，补齐合法 short write，拒绝零进展或非法写入返回值，并核对真实产物增长。普通未跟踪多文件捕获中的任一路径失败都会回滚整个批次。完整 patch 保留在磁盘，模型侧状态对每种 patch 视图最多读取 24,000 原始字节并附加标记，避免 decode/JSON 放大。只有评估内存、磁盘和审查成本后，才应调整 `--max-delta-bytes` 或 `--max-untracked-patch-bytes`。任何不完整捕获都会在自动 Review 前停止；被截断的源内容不能获得 PASS。
...
Protected 删除产生的退出 10 可通过 `review_decision.py --repo ... --run-dir ... --reviewer ... --decision accept|reject` 处理。Human Review v4 只接受精确的 `rootloom-human-review-result-v1` Envelope：非空规范 protected deletion 必须是完整的零修复变更集，仍被 allowed_paths 授权并保持 metadata-only，Git index 未变，且 metadata/Delta Artifact、Runner 与 Run 身份均已绑定。它绑定规范化 Run 路径与 mode 0700 目录身份，并在任何内容指纹前把完整的最终验证 metadata-only 下限传入每次捕获；初次 Binding 必须与该最终验证仓库承诺精确一致，而不能吸收稍后的状态。它重新核对规范化最终 Result 核心、受限的可见内容与 metadata-only 承诺、status/路径集合、Git index/control 状态、每个 protected 目标的 exact-missing 状态与词法父边界，以及私有 Artifact 哈希。Artifact 哈希默认限制为单文件 64 MiB、合计 512 MiB、256 个文件和 120 秒；实际观察字节会在读取源头即时消耗单文件及剩余合计额度。每次 Result 与评审 Artifact 读取都会同时证明描述符元数据稳定，且规范目录项仍指向该描述符；Artifact 枚举还会在哈希前后承诺完全一致的名称集合。终态 NDJSON 与 Summary JSON 都必须是 pinned Run Directory 下的普通单链接文件，并在追加、fsync、最终校验和补偿期间保持 pinned descriptor。提交门禁会通过这些固定描述符重新读取精确预期字节数，并要求规范名称、inode、单链接、mode、size 与 SHA-256 全部等于 Rootloom 的原始 Payload。任一失败都会先把两个 pinned 原始 inode 截断为空，使重试没有歧义。

可用 `review_decision.py verify --repo ... --run-dir ...` 对既有 Decision Pair 做只读审计。stdout 精确保持为 `VALID`（退出 0）、`INVALID`（退出 9）、`STALE`（退出 12）或 `UNVERIFIED`（退出 13），非 VALID 状态会在 stderr 输出一条有界且不含 Artifact 字节的原因。`INVALID` 表示持久化证据格式、安全、规范性或内部一致性无效；`STALE` 只表示结构有效且完整重捕获后当前仓库、Artifact 或 protected 状态不同；`UNVERIFIED` 表示 Git 启动、权限/I/O、拓扑、Deadline 或本地资源上限令验证无法得出结论。重捕获会关闭 Git Optional Lock、fsmonitor 与 untracked cache 写入。持久化 Policy 只是生产证据，不能决定消费成本；独立本地默认上限为单 Artifact 64 MiB、合计 512 MiB、200,000 个状态路径、16 MiB 状态元数据、50,000 个 ignored 路径与 120 秒，只能通过对应 verify 参数显式修改。

Producer 与 Consumer 对 Terminal 和 Summary 各自共用 1 MiB 上限；两份 Payload 都会在创建任一路径前完成预检，reviewer 与 local account 必须规范、无控制字符且不超过 4096 个 UTF-8 字节，local UID 只能为空或非负，决定时间必须是规范 UTC。既有 v2/v3 Binding、Envelope 之前的 Result 与旧版非空 Terminal 但缺少 Summary 的状态继续关闭式失败。Human Review v4 仍冻结为可信个人或小团队环境下的可归因人工终局；它不是本地对抗型审批、组织身份密码学证明或 WORM 存储。更高保证应由只读快照、独立 Worker 或 UID、外部签名者、远端不可变 Artifact Store 与 WORM 审计基础设施承担。
...
- 全局 setup 是显式、进程加锁、预写恢复清单、哈希校验、权限模式保留且可回滚的，并会补偿可捕获的 apply/rollback 失败。Setup、项目指导与 Strict Runner 共用一个加固协作式锁入口：在截断或写入 owner 数据前拒绝最终 symlink/reparse point、hardlink、作为 symlink 的直接锁父目录、非普通文件和身份替换。这是基于 inode 的协作，不是不可替换租约或敌对进程互斥；同 UID 进程仍可 rename/unlink 已锁 inode、重建同路径，并独立锁住新 inode。Apply 与 rollback 都会在第一次修改前记录 prepared/applying/committed/compensated/recovered 阶段。新 Manifest 会记录 producer version、recovery schema version 与 target type；Recovery 保留 1.2.12 隐式 target schema 的字面 reader，不再只查询当前插件 catalog。`setup_rootloom.py recover` 只接受被 Manifest 哈希绑定、路径唯一且属于历史托管 target 的完整计划，并在任何写入前验证全部备份、Hash、Mode 与当前前/后状态。原子 rename 以下的存储损坏与持久性仍属于外部边界。
