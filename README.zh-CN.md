...
严格 Runner 保持离线。应在运行前收集已授权的运行时证据，只传入受限且脱敏的材料。证据来源能提高可审计性，但不能证明诊断正确。
...
普通 CI 验证固定的 Codex CLI 基线；另一条定时工作流探测最新 CLI，在维护者审查并正式采用上游变化前只提供信息。这样既能尽早发现漂移，也不会让每次发布依赖未固定工具链。详见[成熟度、保证边界与兼容性](docs/maturity.zh-CN.md)。
