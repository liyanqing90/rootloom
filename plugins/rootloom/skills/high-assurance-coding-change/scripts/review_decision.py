...
def decide(repo: Path, run_dir: Path, reviewer: str, decision: str) -> dict[str, object]:
    ...
    if expected != current:
        raise runner.PipelineError("human review binding drifted; acceptance refused", 9)
    ...
    binding_payload = json.dumps(expected, sort_keys=True, separators=(",", ":")).encode()
    record = {
        "format": "rootloom-human-review-decision-v2",
        "decision": decision,
