...
Resolve protected-deletion exit 10 with `review_decision.py --repo ... --run-dir ... --reviewer ... --decision accept|reject`. Human Review v3 rechecks the canonical final Result core, a bounded commitment over visible content fingerprints, metadata-only fingerprints, status/path sets, the Git index and Git control state, each protected target's exact-missing state and lexical parent boundaries, plus reviewed private Artifact hashes captured through stable directory-relative no-follow descriptors. The command re-reads Result under the repository lock, appends the terminal record, then repeats the complete commitment before success; detected write-window drift compensates that record. It records local account/uid plus the asserted reviewer and rejects a second terminal decision. Existing v2 results are not silently upgraded. The lock serializes cooperative Rootloom writers only, so this remains attributable local approval—not cryptographic organizational identity, WORM storage, or protection from an arbitrary external process after the final check.
