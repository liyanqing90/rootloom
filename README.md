...
Resolve protected-deletion exit 10 with `review_decision.py --repo ... --run-dir ... --reviewer ... --decision accept|reject`. Human Review v4 accepts only an exact `rootloom-human-review-result-v1` Envelope: nonempty canonical protected deletions must be the complete zero-repair change set, remain allowed and metadata-only, preserve the Git index, and reference bound metadata/Delta Artifacts plus Runner and Run identity. It binds the canonical run path and mode-0700 directory identity and carries the complete final validated metadata-only floor into every capture before any content fingerprinting. Initial Binding must exactly match that final validated repository commitment instead of adopting a later state. It rechecks the canonical final Result core, bounded visible-content and metadata-only commitments, status/path sets, Git index/control state, each protected target's exact-missing state and lexical parent boundaries, plus private Artifact hashes. Artifact hashing defaults to 64 MiB per file, 512 MiB total, 256 files, and 120 seconds; observed bytes consume the per-file and remaining aggregate allowances at the read source. Every Result and reviewed Artifact read proves both stable descriptor metadata and that its canonical directory entry still names that descriptor, while Artifact enumeration commits the same exact name set before and after hashing. Terminal NDJSON and Summary JSON are both created/opened as regular single-link files under the pinned Run Directory and retain pinned descriptors through append, fsync, final validation, and compensation. The commit gate re-reads their exact expected byte counts through those fixed descriptors and requires canonical name, inode, one link, mode, size, and SHA-256 to match Rootloom's original payloads. Any failure first truncates both pinned originals to empty files, making a new attempt unambiguous.

Audit an existing pair without modifying it with `review_decision.py verify --repo ... --run-dir ...`. Stdout is exactly `VALID` (exit 0), `INVALID` (exit 9), `STALE` (exit 12), or `UNVERIFIED` (exit 13); non-valid results also place one bounded reason on stderr without serializing Artifact bytes. `INVALID` means persisted evidence is malformed, unsafe, noncanonical, or internally inconsistent. `STALE` is reserved for a complete structurally valid recapture whose repository, Artifact, or protected state differs. `UNVERIFIED` means Git launch, permission/I/O, topology, deadline, or local resource ceilings prevented a conclusion. Before recapture, the verifier validates the exact Result Envelope and v4 Binding, canonical Decision Pair, cross-field counts, mode, paths, hashes, and Artifact references. Repository recapture disables optional Git locks, fsmonitor, and untracked-cache writes. Persisted policy is evidence rather than authority: independent local ceilings default to 64 MiB per Artifact, 512 MiB total, 200,000 state paths, 16 MiB state metadata, 50,000 ignored paths, and 120 seconds, and can be changed only with the corresponding verifier flags.

The producer and consumer share a 1 MiB limit for each Terminal and Summary. Both payloads are preflighted before either path is created; reviewer and local-account identities are canonical, control-free, and limited to 4096 UTF-8 bytes; local UID is null or nonnegative; and decision time is canonical UTC. Existing v2/v3 bindings, pre-Envelope Results, and older non-empty Terminal-without-Summary states fail closed. Human Review v4 remains deliberately frozen as attributable final review for trusted personal or small-team environments. It is not a hostile local approval system, cryptographic organizational identity, or WORM storage; stronger guarantees belong to read-only snapshots, separate workers or UIDs, external signers, remote immutable Artifact stores, and WORM audit infrastructure.
