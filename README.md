...
Audit an existing pair without modifying it with `review_decision.py verify --repo ... --run-dir ...`. Stdout is exactly `VALID` (exit 0), `INVALID` (exit 9), or `STALE` (exit 12); invalid and stale results also place one bounded reason on stderr without serializing Artifact bytes. Before any repository or reviewed-Artifact recapture, the verifier validates the exact v4 Binding field set, repository and Run Directory identities, Git/Result hashes, canonical state policy and path lists, repository commitment format and hashes, Artifact map and persisted budgets, protected-deletion commitments, plus Binding/Result agreement. It then checks one canonical terminal record, canonical Summary agreement including reviewer/decision/binding and `decision_record_sha256`, and regular single-link files. Only structurally valid evidence whose current repository, Artifact, or protected state changed is `STALE`; every malformed, unsafe, noncanonical, or internally inconsistent input is `INVALID`/9.

The producer and consumer share a 1 MiB limit for each Terminal and Summary. Both payloads are preflighted before either path is created, reviewer and local-account identities are limited to 4096 UTF-8 bytes, and every committed pair must be immediately readable by the verifier. Existing v2/v3 results and older non-empty Terminal-without-Summary states fail closed. Human Review v4 remains deliberately frozen as attributable final review for trusted personal or small-team environments. It is not a hostile local approval system, cryptographic organizational identity, or WORM storage; stronger guarantees belong to read-only snapshots, separate workers or UIDs, external signers, remote immutable Artifact stores, and WORM audit infrastructure.
