...
python3 <high-assurance-skill-dir>/scripts/run_pipeline.py \
  ...
  --bind-verification-path scripts/acceptance.sh \
  --verify 'make focused-test' \
  --verify 'make check'
```

The runner loads the same four Agent TOMLs and enforces a repository lock, clean baseline, read-only stage snapshots, one writer, exact allowed paths, unchanged Git index, structured outputs, deterministic verification, independent review, and at most one repair cycle. It also fingerprints detected verification entrypoints before the writer and rejects post-writer changes before verification can run. Bound entrypoints include explicit repository-relative verification path tokens, operator-supplied `--bind-verification-path` harnesses, `make` files, JavaScript package manifests, pytest configuration files, missing common candidates that could take precedence later, and repository-internal symlink chains plus final target contents. It supports Linux, macOS, and WSL; native Windows is not supported. Artifacts are private and must live outside the target repository.
