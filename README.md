...
`analyze_change.py --write-baseline ...` remains available for analyzer-only/self-declared evidence, but it does not become intake-sealed without the intake and seal lifecycle above. New intakes write `rootloom-change-baseline-v3`; compatible legacy baseline v2 evidence remains readable and sealable.
